...
public static class IntPhrasePair
{
    ...
    public boolean equals(Object obj) {
        ...
        if (obj == null) return false;
        if (getClass() != obj.getClass()) return false;
        AlignmentTester.IntPhrasePair other = (AlignmentTester.IntPhrasePair) obj;
    ...
    private static PhraseTable getPhraseTableFromAlignedCorpus(Iterable<SentencePair> trainingSentencePairs, WordAligner wordAligner, Counter<String> weights) {
        ...
        HashMap<AlignmentTester.IntPhrasePair, Integer> counter = new HashMap<AlignmentTester.IntPhrasePair, Integer>(1000, 0.5f);
        int sent = 0;
        for (SentencePair sentencePair : trainingSentencePairs) {
            ...
            List<AlignmentTester.IntPhrasePair> phrases = phraseExtractor.extract(al, lowercase(sentencePair), maxPhraseSize, englishWordIndexer, foreignWordIndexer,
                    maxNumUnaligned);
            for (AlignmentTester.IntPhrasePair pair : phrases) {
                increment(counter, pair, 1);
            }

        }
        Logger.endTrack();
        HashMap<AlignmentTester.IntArrayWrapper, Integer> eCounter = new HashMap<AlignmentTester.IntArrayWrapper, Integer>(1000, 0.5f);
        HashMap<AlignmentTester.IntArrayWrapper, Integer> fCounter = new HashMap<AlignmentTester.IntArrayWrapper, Integer>(1000, 0.5f);

        Logger.startTrack("Counting extracted phrases");
        int phraseNum = 0;
        for (java.util.Map.Entry<AlignmentTester.IntPhrasePair, Integer> pair : counter.entrySet()) {
            if (phraseNum % 100000 == 0) System.out.println("Phrase " + phraseNum + " of " + counter.size());
            phraseNum++;
            int count = pair.getValue();
            AlignmentTester.IntPhrasePair key = pair.getKey();

            increment(eCounter, new AlignmentTester.IntArrayWrapper(key.english), count);
            increment(fCounter, new AlignmentTester.IntArrayWrapper(key.foreign), count);
        }
        Logger.endTrack();
    ...
    private static void doDecoding(Decoder decoder, Iterable<List<String>> frenchSentences, Iterable<List<String>> englishSentences, List<BleuScore> scores,
        ...
        for (Pair<List<String>, List<String>> input : CollectionUtils.zip(Pair.makePair(frenchSentences, englishSentences))) {
            ...
            double score = Decoder.StaticMethods.scoreHypothesis(hyp, languageModel, dm);
